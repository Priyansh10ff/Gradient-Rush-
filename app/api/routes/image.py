...
from app.services.image_processor import (
    ImageDependencyUnavailableError,
...
@router.post("/image", response_model=UploadProcessingResult)
async def upload_image(file: UploadFile = File(...)) -> UploadProcessingResult:
    """Persist an image, extract local OCR/metadata, and index the results."""
    receipt = acknowledge_upload(file, MediaModality.IMAGE)
    source_path = await persist_upload(file, receipt.source_asset.source_id)
    try:
        result = await process_image(source_path, receipt.source_asset)
