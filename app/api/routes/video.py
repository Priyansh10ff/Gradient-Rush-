...
@router.post("/video", response_model=UploadProcessingResult)
async def upload_video(file: UploadFile = File(...)) -> UploadProcessingResult:
    """Persist a video and asynchronously extract frames plus a transcript timeline."""
    ...
    try:
        indexed_count = await index_records(result.extracted_knowledge)
